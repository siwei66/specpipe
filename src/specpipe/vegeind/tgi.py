# -*- coding: utf-8 -*-
"""
SpecPipe - Vegetation Indices - TGI (Triangular Greenness Index)
...
def tgi(
    ...
    """
    Compute TGI (Triangular Greenness Index) of 2D array-like of 1D spectra data series.
    ...
    # red_edge3 = (773, 793)
    # nir_range = (785, 900)
    if (min(wavelength) > blue_range[0]) | (max(wavelength) < red_range[1]):  # Adjust according to applied VI
        raise ValueError(
            f'Index requires wavelength range ({blue_range[0]}, {red_range[1]}) '  # Adjust according to applied VI
    ...
    if (
        np.prod(blue != 0) * np.prod(green != 0) * np.prod(red != 0) *
        # np.prod(re1 != 0) *
        # np.prod(re2 != 0) *
        # np.prod(re3 != 0) *
        # np.prod(nir != 0) *
    ...
    tgi_values = -0.5 * (
        (sum(red_range) / 2 - sum(blue_range) / 2) * (red - green)
        - (sum(red_range) / 2 - sum(green_range) / 2) * (red - green)
    )
    ...
    if axis == 0:
        df_vi = pd.DataFrame(vi_arr.T, columns=vi_names).loc[:, ['TGI']]
    else:
        df_vi = pd.DataFrame(vi_arr, index=vi_names).loc[['TGI'], :]
