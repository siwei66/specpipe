# -*- coding: utf-8 -*-
"""
SpecPipe - Vegetation Indices - LCI (Leaf Chlorophyll Index)
...
def lci(
    ...
    """
    Compute LCI (Leaf Chlorophyll Index) of 2D array-like of 1D spectra data series.
    ...
    if axis == 0:
        df_vi = pd.DataFrame(vi_arr.T, columns=vi_names).loc[:, ['LCI']]
    else:
        df_vi = pd.DataFrame(vi_arr, index=vi_names).loc[['LCI'], :]
    return df_vi
