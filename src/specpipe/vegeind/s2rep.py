# -*- coding: utf-8 -*-
"""
SpecPipe - Vegetation Indices - S2REP (Sentinel-2 Red Edge Position)
...
def s2rep(
    ...
    """
    Compute S2REP (Sentinel-2 Red Edge Position) of 2D arraylike of 1D spectra data series.
    ...
    red_edge3 = (773, 793)
    # nir_range = (785, 900)
    if (min(wavelength) > red_range[0]) | (max(wavelength) < red_edge3[1]):  # Adjust according to applied VI
        raise ValueError(
            f'Index requires wavelength range ({red_range[0]}, {red_edge3[1]}) '  # Adjust according to applied VI
    ...
    if axis == 0:
        df_vi = pd.DataFrame(vi_arr.T, columns=vi_names).loc[:, ['S2REP']]
    else:
        df_vi = pd.DataFrame(vi_arr, index=vi_names).loc[['S2REP'], :]
