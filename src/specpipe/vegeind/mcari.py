# -*- coding: utf-8 -*-
"""
SpecPipe - Vegetation Indices - MCARI (Modified Chlorophyll Absorption Reflectance Index)
...
def mcari(
    ...
    """
    Compute MCARI (Modified Chlorophyll Absorption Reflectance Index) of 2D array-like of 1D spectra data series.
    ...
    # red_edge3 = (773, 793)
    # nir_range = (785, 900)
    if (min(wavelength) > green_range[0]) | (max(wavelength) < red_edge1[1]):  # Adjust according to applied VI
        raise ValueError(
            f'Index requires wavelength range ({green_range[0]}, {red_edge1[1]}) '  # Adjust according to applied VI
    ...
    if (
        # np.prod(blue != 0) *
        np.prod(green != 0) * np.prod(red != 0) * np.prod(re1 != 0) *
        # np.prod(re2 != 0) *
        # np.prod(re3 != 0) *
        # np.prod(nir != 0) *
        1
    ...
    if axis == 0:
        df_vi = pd.DataFrame(vi_arr.T, columns=vi_names).loc[:, ['MCARI']]
    else:
        df_vi = pd.DataFrame(vi_arr, index=vi_names).loc[['MCARI'], :]
