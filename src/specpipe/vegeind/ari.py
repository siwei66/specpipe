# -*- coding: utf-8 -*-
"""
SpecPipe - Vegetation Indices - ARI (Anthocyanin Reflectance Index)
...
def ari(
    ...
    vi_values = 1 / band1_values - 1 / band2_values
    vi_values = np.array([vi_values])

    if axis == 0:
        df_vi = pd.DataFrame(vi_values.T, columns=['ARI'])
    else:
        df_vi = pd.DataFrame(vi_values, index=['ARI'])
