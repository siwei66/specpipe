# -*- coding: utf-8 -*-
"""
SpecPipe - Vegetation Indices - SR NIR/Red (Simple Ratio NIR/Red)
...
def sr_nir_r(
    ...
    """
    Compute SR NIR/Red (Simple Ratio NIR/Red) of 2D array-like of 1D spectra data series.
    ...
    if axis == 0:
        df_vi = pd.DataFrame(vi_arr.T, columns=vi_names).loc[:, ['SR_NIR_to_R']]
    else:
        df_vi = pd.DataFrame(vi_arr, index=vi_names).loc[['SR_NIR_to_R'], :]
    return df_vi
