# -*- coding: utf-8 -*-
"""
SpecPipe - Vegetation Indices - MSAVI (Modified Soil Adjusted Salinity Index)
...
def msavi(
    ...
    """
    Compute MSAVI (Modified Soil Adjusted Salinity Index) of 2D array-like of 1D spectra data series.
    ...
    vi_values = 0.5 * (2 * band2_values + 1 - ((2 * band2_values + 1) ** 2 - 8 * (band2_values - band1_values)) ** 0.5)
    vi_values = np.array([vi_values])

    if axis == 0:
        df_vi = pd.DataFrame(vi_values.T, columns=['MSAVI'])
    else:
        df_vi = pd.DataFrame(vi_values, index=['MSAVI'])
