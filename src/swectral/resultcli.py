# -*- coding: utf-8 -*-
"""
Swectral - File results reporters for console interface
...
# ------
# Core result of each chain to dill (.__swectral_dill_data/.__swectral_core_result_<file name>.dill)
# Groupstats result to dill (.__swectral_dill_data/.__swectral_result_summary_<file name>.dill)
...
# <name>__swectral__.dill scaning and directly read back the corresponding data -> as a dict
...
def group_stats_report(specpipe_report_dir: str) -> dict:
    ...
    data_paths = _get_file_path(specpipe_report_dir, ".__swectral_result_summary_")
    for data_path in data_paths:
        data_name = str(os.path.basename(data_path)).replace(".__swectral_result_summary_", "").replace(".dill", "")
...
def core_chain_report(specpipe_report_dir: str) -> list[dict]:
    ...
    data_dir_paths = _get_dir_path(specpipe_report_dir, ".__swectral_dill_data")

    # Filter chain dir paths
    chain_dir_paths = []
    for dir_path in data_dir_paths:
        if ('test_run' not in dir_path) and (
            ".__swectral_core_result_Chain_process_info.dill" in os.listdir(unc_path(dir_path))
        ):
    ...
    for dir_path in chain_dir_paths:
        ...
        chain_report_paths = _get_file_path(dir_path, ".__swectral_core_result_")

        # Get chain info
        chain_info_path = dir_path + "/.__swectral_core_result_Chain_process_info.dill"
        ...
        for data_path in chain_report_paths:
            data_name = str(os.path.basename(data_path)).replace(".__swectral_core_result_", "").replace(".dill", "")
            # Remove model name
