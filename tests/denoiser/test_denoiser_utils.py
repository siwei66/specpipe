...
class TestRollWindow:
    ...
    def test_apply_axes() -> None:
        ...
        # Axis 0
        res_roll_axis0 = RollWindow(5, axis=0).apply(arr, func_for_apply)
        assert isinstance(res_roll_axis0, np.ndarray)
        assert res_roll_axis0.shape == (10, 12)
        # Axis 1
        res_roll_axis1 = RollWindow(5, axis=1).apply(arr, func_for_apply)
    ...
    def test_window_sizes() -> None:
        """Test different rolling axes."""
        arr = create_test_array(shape=(14, 12))
        res_roll_window1 = RollWindow(2, axis=0, roll_mode='center').apply(arr, func_for_apply)
        assert isinstance(res_roll_window1, np.ndarray)
        assert res_roll_window1.shape == (13, 12)
        res_roll_window2 = RollWindow(3, axis=0, roll_mode='center').apply(arr, func_for_apply)
        assert isinstance(res_roll_window2, np.ndarray)
        assert res_roll_window2.shape == (12, 12)
        res_roll_window3 = RollWindow(4, axis=0, roll_mode='center').apply(arr, func_for_apply)
        ...
        with pytest.raises(ValueError, match='window_size must be at least 2'):
            _ = RollWindow(1, roll_mode='center').apply(arr, func_for_apply)

    @staticmethod
    def test_roll_modes() -> None:
        """Test different rolling modes."""
        arr = create_test_array(shape=(14, 12))
        res_roll_mode1 = RollWindow(5, axis=0, roll_mode='center').apply(arr, func_for_apply)
        assert isinstance(res_roll_mode1, np.ndarray)
        assert res_roll_mode1.shape == (10, 12)
        res_roll_mode2 = RollWindow(5, axis=0, roll_mode='end').apply(arr, func_for_apply)
        assert isinstance(res_roll_mode2, np.ndarray)
        assert res_roll_mode2.shape == (10, 12)
        res_roll_mode3 = RollWindow(5, axis=0, roll_mode='knn').apply(arr, func_for_apply)
    ...
    def test_padding_approaches() -> None:
        ...
        arr = create_test_array(shape=(14, 12))
        res_roll_pad1 = RollWindow(5, axis=0, roll_mode='center', padding='none').apply(arr, func_for_apply)
        assert isinstance(res_roll_pad1, np.ndarray)
        assert res_roll_pad1.shape == (10, 12)
        res_roll_pad2 = RollWindow(5, axis=0, roll_mode='center', padding='nan').apply(arr, func_for_apply)
        assert isinstance(res_roll_pad2, np.ndarray)
        assert res_roll_pad2.shape == (14, 12)
        res_roll_pad3 = RollWindow(5, axis=0, roll_mode='center', padding='ext_edge').apply(arr, func_for_apply)
        assert isinstance(res_roll_pad3, np.ndarray)
        assert res_roll_pad3.shape == (14, 12)
        res_roll_pad4 = RollWindow(5, axis=0, roll_mode='center', padding='constant_edge').apply(arr, func_for_apply)
        assert isinstance(res_roll_pad4, np.ndarray)
        assert res_roll_pad4.shape == (14, 12)
        res_roll_pad5 = RollWindow(5, axis=0, roll_mode='center', padding='extrapolation').apply(arr, func_for_apply)
    ...
    def test_chain_apply() -> None:
        ...
        arr = create_test_array(shape=(14, 12))
        res_roll = RollWindow(5, axis=0).apply(arr, func_for_apply)
        res_roll_chain = RollWindow(5, axis=0).chain_apply(arr, [func_for_apply])
        assert np.allclose(res_roll, res_roll_chain)
        res_roll_chain1 = RollWindow(5, axis=0).chain_apply(arr, [func_for_apply, func_for_apply1])
        ...
        res_roll_chain2 = window_roller.chain_apply(arr, [func_for_apply, func_for_apply1])
